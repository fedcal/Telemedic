...
4. **A versioned register of system identifiers exists** that declares, for each identifier recognised, whether it is accepted on input, whether it is emitted on output and with what precedence. It lives in the anti-corruption layer of the frontier context and **never in the domain** (constraint [V-142](../11_registri/01-vincoli-in-vigore.md#v-142) of the domain area). With this form the choice is a configuration value and not a migration.
