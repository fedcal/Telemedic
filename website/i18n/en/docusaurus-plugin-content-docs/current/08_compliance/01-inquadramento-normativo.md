...
**What the project produces and the role it intends to assume.** From `D58` the project
intends to assume the role of manufacturer, and with `D63` the marking at the end of a completed
conformity assessment is a **product requirement**; the legal entity is still to be constituted. Decision
`D58` widens the scope of decision `D49`: the project produces and publishes the technical file,
the lifecycle documentation under IEC 62304, the risk management under ISO 14971 and the usability
engineering file under IEC 62366-1 as material that makes the conformity assessment path possible.
When the manufacturer entity is constituted, it will assume the acts reserved to that role: it will
engage notified bodies, conduct the clinical evaluation and affix the CE marking. Today the project
bears no CE marking, is not covered by any declaration of conformity and is not usable for the
delivery of healthcare services to real patients. The reason why this material **cannot be recovered
after the fact** (`D45`) is the same as always: its absence would make it impossible for *anyone*
to certify later, regardless of who assumes the manufacturer role. Chapter
