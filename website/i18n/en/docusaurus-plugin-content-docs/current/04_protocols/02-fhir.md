...
[V2](../11_registri/03-vincoli-fondanti.md#v2) requires that what Telemedic persists be content drafted by the professional, not information
...
constraint [V-04](../11_registri/01-vincoli-in-vigore.md#v-04), and it belongs to the security area.
...
constraint [V-01](../11_registri/01-vincoli-in-vigore.md#v-01): a service may take place without media, with several sessions or with failed
...
> **Exact form of the address sub-extension** `[NV]` to be verified by the protocols area: the published
> definition exposes `address` as a **complex extension** over an extended contact type, not as a simple
> value. The precise form of its sub-extensions has not been verified element by element. Before writing
> an address into
...
directly satisfies constraint [V3](../11_registri/03-vincoli-fondanti.md#v3) on total integrability. Project rules:
...
under constraint [V-03](../11_registri/01-vincoli-in-vigore.md#v-03) - **the bindings that depend on that terminology are not validated**. The
...
> **Concrete validation tools** `[NV]` to be established by the technical area: the names, versions
> and invocation modes of the tools for validating and publishing the guides have not been verified against
> primary sources. They must be established and pinned in the build configuration. **To be asked of**:
...
resource that means untracked data loss, incompatible with constraint [V5](../11_registri/03-vincoli-fondanti.md#v5). The cost is that it
...
> **[Q-06](../11_registri/02-questioni-aperte.md#q-06) - open question towards the architecture area and the technical area. This area does not
...
**negotiated at runtime and never assumed** - that is constraint [V-11](../11_registri/01-vincoli-in-vigore.md#v-11), and the technical reason
