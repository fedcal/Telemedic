...
(constraint `V-188`: every integration test exercises at least two tenants and two distinct integrators, with deliberately divergent configurations).
...
**replaceable** and not hardcoded (constraint `V-07`). It is the only form that survives the fact that the document models, the typology codes and the indexing metadata of those ten
typologies **are not yet publicly available** (question `Q-07`, open).
...
A telemedicine system that does not communicate with the management system already in use will not be adopted, however good it is. Hence the architectural constraint `V3` - **every capability of the system must be reachable by a third party system via documented interface; no functionality is accessible only from the user interface** - and its operational corollary `V-164`: the area that introduces a capability introduces also the contract, and as long as the contract does not exist the capability is not complete.
...
`EX-NOSHOW` and `EX-TECH-PATIENT` share the same terminal state and have **opposite** administrative effects, and no area can collapse them into a single field (constraint `V-141`).
...
and silence is never treated as normality (constraint `V-09`).
...
The domain model is written **entirely** on the second formulation, and no project artefact -documentation, interface, public material, class name or event name - can use «real-time monitoring», «continuous surveillance» or equivalent formulas (constraint `V-144`).
It is not editorial caution: it is the choice on which the entire downstream regulatory path depends. The **formal freezing was deliberate** (`D55`, which closes question `Q-144`), and from it descends a permanent ban: **no function can be added if it shifts the system toward clinical real time**, and the evaluation must be made before writing the function, not after.

### 3.4 Data sovereignty as a verifiable property, not as a slogan

Constraint `V1` establishes that **no mandatory component of the main pathway depends on services that are not replaceable or established outside the European Union**. Three deployment profiles are documented and supported - European Union, Italian territory, qualified national cloud - and no runtime dependency can prevent the most restrictive profile (`D24`).
...
The same logic applies in the negative: the external terminology service, if established outside the Union, is a transfer at the moment it receives data referring to a patient. The project's answer is not to place it elsewhere but **never to send it patient identifiers** - sovereignty is satisfied by absence of datum (constraint `V-151`).
...
No external identifier is a primary key, and normalisation happens at the boundary, never in the domain (constraint `V-142`).
...
amount (constraint `V-166`). It is the exclusion **not reopenable** as long as the source is in force.
...
CT** and no main pathway can require it (constraint `V-03`). The cost is declared instead of being concealed.
...
they contain the analysis from which a controlled document is written (constraint `V-174`). It is the point of junction of the entire model: the project can fill almost the entire technical file, except what presupposes a document control system of a manufacturer.
...
The first is technical in nature (constraint `V-133`, complete list in
...
The second is regulatory in nature (constraint `V-171`, table in
...
A fourth reformulation concerns the session quality index: it is **proprietary and must be declared as such**, because no average opinion score exists applicable to real time according to any international recommendation (constraint `V-114`).
...
(`D45`). Hence constraint `V-182`: **no row of application code precedes the build chain** that generates the bill of materials and the register of identifiers.
