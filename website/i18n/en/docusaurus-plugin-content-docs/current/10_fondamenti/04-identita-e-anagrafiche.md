...
> of writing» and declares to whom verification must be requested. Every `[NV]` carries, within
> the same paragraph, the indication of the recipient in one of three permitted forms: the code of
> an area in backticks, a question identifier, or an external party named according to the rules
> in `CONTRIBUTING.md`. § 12 collects all the points so marked, with an indication of where the
> missing information should be requested. When a statement is a **project proposal** and not a
...
tenant-awareness constraint declared in the context ([V4](../11_registri/03-vincoli-fondanti.md#v4)):
...
- the regulation provides that the Agency publish **the level to be associated with homogeneous
  categories of service**, but such document `[NV]` must be requested from the Digital Italy Agency;
...
Diamond **[V2](../11_registri/03-vincoli-fondanti.md#v2)** is the one almost all systems omit, and it is the one that distinguishes a support
